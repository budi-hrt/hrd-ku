...
class Kendaraan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['kendaraan'] = $this->kendaraan->get_all()->result_array();
        $data['tipe'] = $this->db->get('tb_type_kendaraan')->result_array();
        $this->load->view('template/header', $data);
        $this->load->view('template/sidebar');
        $this->load->view('page/master/kendaraan/index', $data);
    }
    ...
    {
        if (isset($_GET['term'])) {
            $result = $this->kendaraan->cari_kendaraan($_GET['term']);
            if (count($result) > 0) {
